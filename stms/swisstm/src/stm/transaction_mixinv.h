...
inline void wlpdstm::TxMixinv::RestartJump() {
    ...
    RestartJumpInC(jmp_buf != NULL ? *jmp_buf : start_buf, LONG_JMP_RESTART_FLAG);
}
...
inline void wlpdstm::TxMixinv::AbortJump() {
    RestartJumpInC(jmp_buf != NULL ? *jmp_buf : start_buf, LONG_JMP_ABORT_FLAG);
}
